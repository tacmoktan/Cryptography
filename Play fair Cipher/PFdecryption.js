let Ciphertext = "FKNZNZXOMLWGLS";
let P = Ciphertext.replace(/ +/g,"").toUpperCase();
let P1 = "";
...
P=P.replace("J","I"); //to replace the J to I in Ciphertext
document.write(`${P} <br>`);
let key = "black";
...
for(let x=0;x<(P.length)/2;x++){
    ...
    // 0 -> row and 1 -> column
    if(c[1]>d[1]){   //first letter's row is greater
        if(c[0] !==d[0]){
        //document.write(c[0]);  
    ...
    else if(c[1]<d[1]){   //second letter's row is greater
        if(c[0] !== d[0]){
           // document.write(c[0]);
           // document.writeln(d[1]);
            numToChar(c[0],d[1]);
